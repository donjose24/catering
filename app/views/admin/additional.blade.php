...
@section ('body')
    ...
    <hr>
{{ Form::open(['action' => ['AdminController@attachAdditionalMenu'], 'files' => true, 'role' => 'form']) }}
    {{ Form::hidden('reservation_id', $reservation->id ) }}
    <div class="row">
      <div class="form-group col-md-3">
        {{ Form::label('item_id', 'Menu') }}
        <select id="item-id" name="item_id" class="form-control">
            <option value="">Select Menu</option>
            @foreach($menu as $menus)
                <option value="{{$menus->id}}">{{ucwords($menus->name)}} </option>
            @endforeach
      ...
      <div class="form-group col-md-3">
        {{ Form::label('price', 'Menu Price') }}
        <div class="input-group">
          <span class="input-group-addon"><i class="fa fa-rub"></i></span>
          {{ Form::text('price','', ['class' => 'form-control', 'item_price','readonly']) }}
        </div>
      </div>
...
{{ Form::open(['action' => ['AdminController@attachAdditionalItem'], 'files' => true, 'role' => 'form']) }}
    {{ Form::hidden('reservation_id', $reservation->id ) }}
    <div class="row">
      <div class="form-group col-md-3">
...
<table class="table table-responsive table-striped">
    ...
    <tbody>
      @foreach ($reservation->additionals as $item)
        @if($item->category == 1)
        <?php $menu = Menu::find($item->menu_id);  ?>
        <tr>
          <td>{{ $menu->name }}</td>
          <td>{{ $menu->scat }}</td>
          <td>{{ $menu->price }}</td>
          <td>{{ $item->quantity }}</td>
          <td>{{ ($item->quantity * 1) * ($menu->price * 1) }}</td>

            <td>
              {{ Form::open(['action' => ['AdminController@detachAdditionalMenu'], 'files' => true, 'role' => 'form']) }}
                  {{ Form::hidden('item_id', $item->id) }}
                  {{ Form::hidden('line_total', ($item->quantity * 1) * ($menu->price * 1)) }}
                  {{ Form::hidden('reservation_id', $reservation->id) }}
        ...
        <tr>
          ...
          <td>{{ ($item->quantity * 1) * ($menu->average_price * 1) }}</td>
            ...
            <td>
              {{ Form::open(['action' => ['AdminController@detachAdditionalMenu'], 'files' => true, 'role' => 'form']) }}
                  {{ Form::hidden('item_id', $item->id) }}
                  {{ Form::hidden('line_total', ($item->quantity * 1) * ($menu->average_price * 1)) }}
                  {{ Form::hidden('reservation_id', $reservation->id) }}
                    <button type="submit" class="btn btn-danger pull-right">
                      <i class="fa fa-trash-o"></i>
                    </button>
