...
<table class="table table-responsive table-striped">
    ...
    <tbody>
      @foreach ($reservation->returns as $item)
       <?php $menu = Item::find($item->menu_id);  ?>
        <tr>
                  ...
                  <td>{{ ($item->quantity * 1) * ($menu->average_price * 1) }}</td>
                    ...
                    <td>
                      {{ Form::open(['action' => ['AdminController@detachReturnItem'], 'files' => true, 'role' => 'form']) }}
                          {{ Form::hidden('item_id', $item->id) }}
                          {{ Form::hidden('line_total', $item->quantity) }}
                          {{ Form::hidden('reservation_id', $menu->id) }}
